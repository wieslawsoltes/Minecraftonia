...
public sealed class GlobalIlluminationEngine<TBlock>
{
    ...
    public LightingResult ComputeLighting(
        ...
        Func<TBlock, BlockFace, float, float, VoxelMaterialSample> sampleMaterial,
        Vector3 hitPoint,
        Vector3 normal,
        BlockFace face,
        ...
        int bounceDepth)
    {
        ...
        if (_sunIntensity > 0f)
        {
            ...
            if (ndotl > 0f)
            {
                ...
                float visibility = ComputeSunVisibility(world, sampleMaterial, shadowOrigin, _sunDirection);
        ...
        Vector3 sunDirection)
    {
        ...
        {
            return TraceVisibility(world, sampleMaterial, origin, sunDirection, _sunMaxDistance);
        }
        ...
        for (int i = 0; i < tapCount; i++)
        {
            ...
            visibility += TraceVisibility(world, sampleMaterial, origin, sampleDir, _sunMaxDistance);
        }
        ...
        float maxDistance)
    {
        ...
        while (walker.TryStep(out var step))
        {
            ...
            VoxelMaterialSample sample = sampleMaterial(block, step.Face, uv.X, uv.Y);
        ...
        out Vector3 bentNormal)
    {
        ...
        for (int i = 0; i < _sampleCount; i++)
        {
            ...
            while (walker.TryStep(out var step))
            {
                ...
                VoxelMaterialSample bounceSample = sampleMaterial(block, step.Face, uv.X, uv.Y);
